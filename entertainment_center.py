...
def get_trailer_data(data, object_name, flag=0):
    ...
    if flag:
        ...
        return [name, story_line, yt_trailer_url, poster_image_url, release_date]


def main():
    """
    Creates Movie objects and initializes these objects with
    title, story_line, you_tube_trailer_url, poster_image_url, and release_date.

    Also creates Tv series objects and initializes these objects with
    title, story_line, you_tube_trailer_url, poster_image_url, release_date, season.
    """
    ...
    intelligence_s1 = media.TvShow(series_data[0], series_data[1], series_data[2],
                                   series_data[3], series_data[4], series_data[5])

    series_data = get_trailer_data(data_tv_series, "Intelligence: S2", 1)
    intelligence_s2 = media.TvShow(series_data[0], series_data[1], series_data[2],
                                   series_data[3], series_data[4], series_data[5])

    series_data = get_trailer_data(data_tv_series, "Minority Report", 1)
    minority_report = media.TvShow(series_data[0], series_data[1], series_data[2],
                                   series_data[3], series_data[4], series_data[5])
